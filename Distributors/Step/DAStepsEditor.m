...
- (void)viewWillAppear:(BOOL)animated{
    ...
    for (NSInteger section = 0; section< 1+self.steps.count; section++) {
        [self.config setCellCls:[DATaskCell class] inSection:section];
    }
    
    [self.config setCellCls:[DANewStepCell class] inSection:self.SectionExtra];
...
- (NSInteger)numberOfRowsInSection:(NSInteger)section{
    ...
    if ([self isStepSection:section]) {
        DAStep *item = [self.steps objectAtIndex:section];
